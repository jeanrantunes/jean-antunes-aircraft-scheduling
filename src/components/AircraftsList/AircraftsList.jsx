...
const AircraftsList = ({setAircraftSelected, flightsSelected, height}) => {
    ...
    return (
        <>
            ...
            <ScrolableList>
                {isLoading ? 
                    <ContainerLoading><CircularProgress /></ContainerLoading>
                : aircrafts.map(aircraft => (
                    <li key={aircraft.ident}>
                        <AircraftsCard onClick={() => setAircraftSelected(aircraft)}>
                            <Grid>
                                <Row center='xs'>
                                    <Col xs={12}><strong>{aircraft.ident}</strong></Col>
                                    <Col xs={12}><Pecentage>{getPercentageOfUtilization(flightsSelected)}%</Pecentage></Col>
                                </Row>
                            </Grid>
                        </AircraftsCard>
                    </li>
